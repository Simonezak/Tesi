...
def run_GGNN_test(inp_path, use_pressure, use_flow, pressure_keep_frac, flow_keep_frac, model, num_test=100, num_steps=50, leak_area=0.1, num_leaks = 3):
    ...
    for i in range(num_test):
        ...
        score_per_node, leak_nodes = run_single_GGNN_test_episode(inp_path, model, env, num_steps, leak_area, num_leaks, use_pressure, use_flow, pressure_keep_frac, flow_keep_frac, B1, edge2pipe, edge_list, adj_matrix, node2idx)
...
def run_single_GGNN_test_episode(inp_path, model, env, num_steps, leak_area, num_leaks, use_pressure, use_flow, pressure_keep_frac, flow_keep_frac, B1, edge2pipe, edge_list, adj_matrix, node2idx):
...
if __name__ == "__main__":
    ...
    ggnn_path = r"/home/zagaria/Tesi/Tesi/wdn-CW-leak-localization_copy/saved_models/ggnn_PF_flowbalance.pt"
    ...
    run_GGNN_test(inp_path=inp_path, use_pressure=use_pressure , use_flow=use_flow , pressure_keep_frac=pressure_keep_frac, flow_keep_frac=flow_keep_frac, model=model, num_test=NUM_TEST, num_steps=NUM_STEPS, num_leaks = num_leaks)
