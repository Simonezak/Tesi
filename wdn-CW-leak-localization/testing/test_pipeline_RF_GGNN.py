...
from utility.utils_evaluation import leak_detection_error, evaluate_model_across_tests, load_RF, load_GGNN

# ============================================================
# PIPELINE RF + GGNNN
# ============================================================

def run_RF_GGNN_pipeline_tests(inp_path, ggnn, rf, num_test=100, num_steps=50, leak_area=0.1, threshold=0.15, K=10):
    ...
    for i in range(num_test):
        print(f"\n=== PIPELINE RF + GGNN TEST {i+1}/{num_test} ===")

        num_leaks = random.randint(0, 2)

        has_true_leak, has_detected_leak, det_error, score_per_node, leak_nodes = run_single_RF_GGNN_pipeline_episode(inp_path, ggnn, rf, adj_matrix, cols, num_steps, leak_area, threshold, num_leaks)
    ...
    # RF METRICS
    # ============================================================
    # prendo tutti gli episodi con leak, indipendentemente dalla predizione
    ...
    ggnn_metrics = evaluate_model_across_tests(scores_per_test, idx2node, leak_nodes_per_test, K)

    # ============================================================
    # END-TO-END METRICS
    # ============================================================

    #pct_episodes_with_leak = 100 * num_episodes_with_leak / num_test
    pct_all_localized = 100.0 * ggnn_metrics["num_detected_and_all_localized"] / num_episodes_with_leak
    pct_partial_localized = 100.0 * ggnn_metrics["num_detected_and_partial_localized"] / num_episodes_with_leak


    # ============================================================
    # PRINT RESULTS
    # ============================================================
    print("\n================= FINAL PIPELINE RESULTS (RF + GGNN) =================")
    ...
    return accuracy, precision, recall, mean_det_error, ggnn_metrics, pct_all_localized, pct_partial_localized, pct_episodes_with_leak


def run_single_RF_GGNN_pipeline_episode(inp_path, ggnn, rf, adj_matrix, cols, num_steps, leak_area, threshold, num_leaks):
    ...
    if has_true_leak and has_detected_leak:
        ...
        for t in range(env.leak_start_step, len(df_pressure)):
            ...
            with torch.no_grad():
                u_pred = ggnn(attr, adj_matrix).view(-1)
...
if __name__ == "__main__":
    ...
    rf = load_RF(rf_path)
    ggnn = load_GGNN(ggnn_path)

    run_RF_GGNN_pipeline_tests(inp_path=inp_path, ggnn=ggnn, rf=rf, num_test=NUM_TEST, num_steps=NUM_STEPS, threshold=THRESHOLD, K=TOP_K)
