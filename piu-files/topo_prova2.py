...
class TopologicalEdgeLayer(torch.nn.Module):
...
class GGNNWithTopology(torch.nn.Module):
...
def load_topo_model(ckpt_path, inp_path):
    """
    Carica SOLO il modello Topo-GGNN (niente RandomForest).
    Gli iperparametri DEVONO coincidere col training.
    """
    ...
    topo_layer = TopologicalEdgeLayer(L1)

    model = GGNNWithTopology(ggnn, topo_layer, B1)

    # ---- LOAD WEIGHTS ----
    model.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
    model.eval()

    print("[OK] Topo-GGNN caricato correttamente")
...
def run_multiple_tests(
    ...
    # ====================================================
    # VALUTAZIONE FINALE (LESSICOGRAFICA, DENSE RANK)
    # ====================================================
